    ...
    async function loadSettings() {
      try {
        ...
        if (res.ok) {
          ...
          setSettings({
            phone: data["CONTACT_PHONE"] || defaultSettings.phone,
            suratPhone: data["SURAT_PHONE"] || defaultSettings.suratPhone,
            genevaPhone: data["GENEVA_PHONE"] || defaultSettings.genevaPhone,
            email: data["CONTACT_EMAIL"] || defaultSettings.email,
            whatsapp: data["WHATSAPP_NUMBER"] || defaultSettings.whatsapp,
            instagram: data["INSTAGRAM_URL"] || defaultSettings.instagram,
            facebook: data["FACEBOOK_URL"] || defaultSettings.facebook,
            linkedin: data["LINKEDIN_URL"] || defaultSettings.linkedin,
          });
          setIsLoading(false);
          return;
        }
    ...
    <>
      ...
      <main className="flex-grow pt-32 md:pt-40 pb-20 px-margin-mobile md:px-margin-desktop max-w-container-max w-full mx-auto mt-2 grid grid-cols-1 lg:grid-cols-12 gap-16 items-start">
        ...
        <div className="lg:col-span-5 flex flex-col gap-10">
          ...
          {isLoading ? (
            <div className="p-6 bg-surface-container-low border border-outline-variant/30 sharp-clip-path flex flex-col gap-4">
              <h4 className="font-headline-sm text-sm text-emerald-deep font-semibold tracking-wide uppercase flex items-center gap-2 animate-pulse">
              ...
              <ul className="space-y-4">
                {[1, 2, 3, 4].map((i) => (
                  <li key={i} className="flex justify-between items-center border-b border-outline-variant/15 pb-2 last:border-b-0">
                    <div className="h-3 w-24 bg-on-surface-variant/10 rounded-sm animate-pulse" />
          ...
          {isLoading ? (
            <div className="w-full h-[52px] bg-[#25D366]/15 animate-pulse sharp-clip-path" />
          ) : (
          ...
          {isLoading ? (
            <div className="flex items-center gap-5">
              <span className="font-label-caps text-[9px] tracking-widest text-on-surface-variant/30 uppercase animate-pulse">
                Follow Us
              </span>
              <div className="h-[0.5px] flex-grow bg-outline-variant/20 animate-pulse" />
              <div className="flex items-center gap-3">
                {[1, 2, 3, 4].map((i) => (
                  <div key={i} className="w-9 h-9 border border-outline-variant/20 bg-on-surface-variant/5 animate-pulse" />
                ))}
              </div>
            </div>
          ) : (
        ...
        {/* Global Presence Ateliers Network Section */}
        <div className="lg:col-span-12 w-full mt-10 md:mt-16">
